...
#define BIND(b) { \
    ...
    // Tree scene
    BIND(IntBinding::bindSliderAndTextbox(
             ui->recursionDepthSlider, ui->recursionDepthTextbox,
             settings.recursionDepth, 1.f, 12.f))
    BIND(FloatBinding::bindSliderAndTextbox(
             ui->fruitDensitySlider, ui->fruitDensityTextbox,
             settings.fruitDensity, 0.f, 1.f))
    BIND(FloatBinding::bindSliderAndTextbox(
             ui->leafDensitySlider, ui->leafDensityTextbox,
             settings.leafDensity, 0.f, 1.f))
    BIND(FloatBinding::bindSliderAndTextbox(
             ui->branchStochSlider, ui->branchStochTextbox,
             settings.branchStochasticity, 0.f, 1.f))
...
void MainWindow::regenerateTree() {
    m_canvas3D->regenerateTree();
}

void MainWindow::dropFruit() {
    m_canvas3D->dropFruit();
}
