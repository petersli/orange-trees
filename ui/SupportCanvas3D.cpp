...
/** Regenerate the tree and display the updated tree */
void SupportCanvas3D::regenerateTree() {
    m_sceneviewScene->regenerateTree();
    update();
}

void SupportCanvas3D::dropFruit() {
    m_sceneviewScene->dropFruit();
}
...
void SupportCanvas3D::mousePressEvent(QMouseEvent *event) {
    ...
    CS123SceneCameraData cam;
    cam.pos = m_defaultPerspectiveCamera->getPosition();
    cam.look = m_defaultPerspectiveCamera->getLook();
    cam.up = m_defaultPerspectiveCamera->getUp();
    cam.heightAngle = m_defaultPerspectiveCamera->getHeightAngle();
    cam.aspectRatio = m_defaultPerspectiveCamera->getAspectRatio();

    if (event->button() == Qt::LeftButton) {
        if (m_sceneviewScene && !settings.useOrbitCamera){
            m_sceneviewScene->keyPressed(this, &cam, event->x(), event->y());
        }
